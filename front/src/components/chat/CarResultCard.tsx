...
export default function CarResultCard({ car }: { car: CarResult }) {
  ...
  const content = (
    <div className="p-4">
      ...
      {car.image_url && <h4 className="text-body-sm text-pure-light font-medium line-clamp-2 mb-2">{car.title}</h4>}
      ...
      {hasLink && <p className="text-[11px] text-mist-gray/40 mt-3 flex items-center gap-1">Ver anuncio →</p>}
    </div>
  );

  if (hasLink) {
    return (
      <a href={href} target="_blank" rel="noopener noreferrer" className="block max-w-[320px] glass-card p-0 overflow-hidden hover:border-mint-glow/30 transition-all">
        {content}
      </a>
    );
  }
  return <div className="glass-card p-0 overflow-hidden max-w-[320px]">{content}</div>;
}
